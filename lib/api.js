...
function parseRaceResult(race) {
  const results = (race.Results || []).map(r => ({
    ...
    status: r.status,
    finished: /^(Finished|Lapped|\+\d+ Lap)/.test(r.status),
...
export async function fetchLastResult() {
  ...
  return race ? parseRaceResult(race) : null;
}

// Full classification for one race of a given season.
export async function fetchRaceResult(year, round) {
  const json = await getJson(`${BASE}/${year}/${round}/results.json?limit=100`);
  const race = json?.MRData?.RaceTable?.Races?.[0];
  return race ? parseRaceResult(race) : null;
}

// Winner of every race in a season, in one request (round -> winner).
export async function fetchSeasonWinners(year) {
  const json = await getJson(`${BASE}/${year}/results/1.json?limit=100`);
  const races = json?.MRData?.RaceTable?.Races || [];
  const map = {};
  for (const r of races) {
    const w = r.Results?.[0];
    if (w) map[Number(r.round)] = {
