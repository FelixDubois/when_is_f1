...
// The finishing time / difference shown per driver: leader's total time,
// "+gap" for lead-lap finishers, "Lapped"/"+N Lap" for lapped, "DNF" otherwise.
function resultTime(r) {
  if (r.status === 'Finished' && r.time) return r.time;
  if (/lap/i.test(r.status)) return r.status === 'Lapped' ? t('result.lapped') : r.status;
  return t('last.dnf');
}

// Full classification: place · driver · time/difference. Used on past-season
// cards where the user has expanded a race's "Results" section.
export function renderRaceResult(container, last) {
  ...
  container.append(
    ...
    el('table', { class: 'standings result-table' }, [
      el('thead', {}, [el('tr', {}, [
        el('th', { class: 'standings__pos', text: t('standings.pos') }),
        el('th', { text: t('standings.entrant') }),
        el('th', { class: 'result-table__time', text: t('result.time') }),
      ])]),
      el('tbody', {}, last.results.map(r => el('tr', { class: 'standings__row' + (r.finished ? '' : ' is-dnf') }, [
        el('td', { class: 'standings__pos', text: r.pos }),
        el('td', {}, [
          el('span', { class: 'standings__code', text: r.code }), ' ',
          el('span', { class: 'standings__team', text: r.team }),
        ]),
        el('td', { class: 'result-table__time', text: resultTime(r) }),
      ]))),
    ]),
  );
}
